<template>
  <vue-custom-scrollbar class="scroll-area" :settings="{
    ...
    <article class="document-section" v-if="current === 0">
      <div class="container"  v-for="(item,key) in body" :key="key" v-html="item.body"></div>
    </article>
    <article class="document-section" v-else-if="current === 1">
      <div class="container"  v-for="(item,key) in body" :key="key" v-html="item.body_kz"></div>
    </article>
...
export default {
  ...
  async created() {
    this.body = await this.$store.dispatch('localStorage/rulesAdGet');
  },
  head() {
    return {
